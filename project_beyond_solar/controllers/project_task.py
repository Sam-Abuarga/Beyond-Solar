        ...
        for field in kwargs:
            if field.startswith('mppt_var_'):
                ...
                if field_name in ['notes_valid']:
                    request.env['sale.mppt'].browse(int(m_id)).write({mppt_fields[field_name]: kwargs[field] or ''})
                else:
                    request.env['sale.mppt'].browse(int(m_id)).write({mppt_fields[field_name]: float(kwargs[field] or 0)})
